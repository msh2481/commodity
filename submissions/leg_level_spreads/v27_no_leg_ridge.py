"""v26: v25 + third XGBoost member on demeaned leg returns.

Adds a 7th ensemble member: XGBoost fitting PER-DATE DEMEANED leg
returns. Different labels -> different bias, hopefully decorrelates
further.
"""
...
class Model:
    ...
    def predict(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        xgb_lr = self._xgb_leg_raw.predict(dtest)
        if xgb_lr.ndim == 1:
            xgb_lr = xgb_lr.reshape(-1, len(self._leg_cols))
        xgb_leg_raw_target = leg_to_target(xgb_lr)

        xgb_ld = self._xgb_leg_dm.predict(dtest)
        if xgb_ld.ndim == 1:
            xgb_ld = xgb_ld.reshape(-1, len(self._leg_cols))
        xgb_leg_dm_target = leg_to_target(xgb_ld)

        return (
            _zscore(direct_raw) + _zscore(direct_dm)
            + _zscore(xgb_direct_df) + _zscore(xgb_leg_raw_target) + _zscore(xgb_leg_dm_target)
        ) / 5.0
