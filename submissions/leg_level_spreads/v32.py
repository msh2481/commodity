"""v32: v30 + cross-sectional rank-transformed leg labels (9-way).

Adds a 9th member: ridge fitting rank(leg_return) per date across assets
and lags. The metric scores cross-sectional Spearman, so training
against ranks aligns the objective more directly.
"""
...
from features import pair_spread_features, volume_features, wide_features
...
class Model:
    ...
    def _rank_leg_per_lag(self, Y: pd.DataFrame) -> pd.DataFrame:
        """Rank-transform per (date, lag) across assets. Scale to [-1, 1]."""
        out = Y.copy()
        for lag in LAGS:
            lag_cols = [c for c in Y.columns if c[1] == lag]
            sub = Y[lag_cols]
            # rank across columns, pct
            r = sub.rank(axis=1, pct=True) * 2 - 1
            out[lag_cols] = r
        return out
    ...
    def fit(self, X: pd.DataFrame, y: pd.DataFrame) -> None:
        ...
        F_vol = volume_features(self.prices)
        F_direct = pd.concat([F_leg, F_pair, F_vol], axis=1)
        self._F_leg = F_leg
        ...
        Y_leg_rk = np.nan_to_num(
            self._rank_leg_per_lag(Y_leg).to_numpy(dtype=np.float32), nan=0.0, posinf=0.0, neginf=0.0
        )
        ...
        self._ridge_leg_rk = Ridge(alpha=ALPHA_RANK, fit_intercept=True).fit(F_leg_tr_sc, Y_leg_rk)
    ...
    def predict(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        leg_rk = leg_to_target(self._ridge_leg_rk.predict(F_leg_te_sc))
        ...
        return (
            _zscore(leg_raw) + _zscore(leg_dm) + _zscore(leg_rk)
