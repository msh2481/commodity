"""v31: v30 + pair cross-sectional rank features.

Adds pair_xrank60 — for each pair, the percentile rank (among ~370
pairs, per date) of its 60-day log-spread z-score. Focuses direct ridge
on "which pairs are currently at relative extremes."
"""
...
class Model:
    ...
    def fit(self, X: pd.DataFrame, y: pd.DataFrame) -> None:
        ...
        F_rank = pair_rank_features(self.prices, self.tp)
        F_vol = volume_features(self.prices)
        F_direct = pd.concat([F_leg, F_pair, F_rank, F_vol], axis=1)
        self._F_leg = F_leg
