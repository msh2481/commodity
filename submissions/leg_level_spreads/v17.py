"""v17: v7 ensemble + LightGBM per-lag leg model.

LightGBM CPU-side third member. Fits one multi-output LGBM per lag
(106 outputs per lag, 4 fits total). Averaged with ridge ensemble.
"""
...
class Model:
    ...
    def fit(self, X: pd.DataFrame, y: pd.DataFrame) -> None:
        ...
        # LightGBM: per-(asset, lag) single-output models. 424 total. Parallelize by using
        # joblib via sklearn MultiOutputRegressor — but that doesn't share threads efficiently.
        # Fit per-column sequentially but with n_jobs per tree to spread across cores.
        from joblib import Parallel, delayed

        def fit_single(j: int):
            booster = lgb.train(
                LGB_PARAMS,
                lgb.Dataset(F_tr, label=Y_leg_np[:, j]),
                num_boost_round=LGB_ROUNDS,
            )
            return booster

        # Process pool risks memory; use threads for lgb (releases GIL for C training)
        self._lgb_models = Parallel(n_jobs=4, backend="threading")(
            delayed(fit_single)(j) for j in range(Y_leg_np.shape[1])
        )
    ...
    def predict(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        lgb_pred = np.zeros((F_te.shape[0], len(self._leg_cols)), dtype=np.float32)
        for j, m in enumerate(self._lgb_models):
            lgb_pred[:, j] = m.predict(F_te).astype(np.float32)
        ...
        lgb_target = compose_targets(to_leg_df(lgb_pred), self.tp)
        ...
        return (zscore(leg_target) + zscore(direct_df) + zscore(lgb_target)) / 3.0
