"""v25: v24 + second XGBoost on LEG returns (with pair features).

Two XGBoost members:
 - xgb_direct: fits the 424 target columns directly
 - xgb_leg: fits the 424 leg returns, then composes to targets

Both share the expanded (wide + pair) feature set. Increases non-linear
diversity while keeping ridge backbone.
"""
...
class Model:
    ...
    def fit(self, X: pd.DataFrame, y: pd.DataFrame) -> None:
        ...
        # XGB direct: on original targets
        dtrain_direct = xgb.DMatrix(F_direct_tr.astype(np.float32), label=y_raw)
        self._xgb_direct = xgb.train(XGB_DIRECT_PARAMS, dtrain_direct, num_boost_round=XGB_DIRECT_ROUNDS)

        # XGB leg: on leg returns (composes to targets via leg→target)
        dtrain_leg = xgb.DMatrix(F_direct_tr.astype(np.float32), label=Y_leg_raw)
        self._xgb_leg = xgb.train(XGB_LEG_PARAMS, dtrain_leg, num_boost_round=XGB_LEG_ROUNDS)
    ...
    def predict(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        xgb_d = self._xgb_direct.predict(dtest)
        if xgb_d.ndim == 1:
            xgb_d = xgb_d.reshape(-1, len(self._target_cols))
        xgb_direct_df = pd.DataFrame(xgb_d, index=X.index, columns=self._target_cols)[leg_raw.columns]

        xgb_l = self._xgb_leg.predict(dtest)
        if xgb_l.ndim == 1:
            xgb_l = xgb_l.reshape(-1, len(self._leg_cols))
        xgb_leg_target = leg_to_target(xgb_l)

        return (
            _zscore(leg_raw) + _zscore(leg_dm)
            + _zscore(direct_raw) + _zscore(direct_dm)
            + _zscore(xgb_direct_df) + _zscore(xgb_leg_target)
        ) / 6.0
