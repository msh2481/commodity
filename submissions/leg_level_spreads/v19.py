"""v19: final ridge ensemble — v7 (raw labels) + v16 (per-date demeaned labels).

Combines four ridge members:
  1. Leg-ridge on raw leg returns
  2. Direct-ridge on raw competition targets
  3. Leg-ridge on per-(date, lag) demeaned leg returns
  4. Direct-ridge on per-date demeaned competition targets

All four z-scored per target column, equal-weight averaged. Ties the
best of the offline ensemble experiments (~0.0901).
"""
