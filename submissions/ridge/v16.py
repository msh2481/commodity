"""v16: large feature pool (v4 + v12 union) + per-target top-K selection + rank-y.

The selector filters out features that don't correlate with y. Richer input
lets the selector pick the best ~30-40 per target from ~100 candidates.
"""
...
def _build_features(target: str) -> pd.DataFrame:
    v4._prepare()
    v12._prepare()
    f4 = v4._build_target_features(target).add_suffix("_v4")
    f12 = v12._build_target_features(target).add_suffix("_v12")
    return pd.concat([f4, f12], axis=1)
...
class Model:
    ...
    def predict(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        for j, t in enumerate(self._target_cols):
            ...
            feats = _build_features(t).loc[test_index].reindex(columns=spec["cols"])
