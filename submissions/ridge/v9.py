"""v9: rank-y + leg-level z-scores + pair-level correlations.

Build on v6 (rank-y, alpha=1) and add:
  - Leg log-price z-score at windows {20, 60} — mean-reversion signal per leg
  - Leg demeaned (cross-sectional) log-returns at {1, 5, 20} — excess return
  - Pair rolling correlation of 1-day returns at windows {20, 60} — regime
"""
...
def _asset_feats(log_p: pd.Series, xs_blocks: dict[str, pd.Series]) -> pd.DataFrame:
    ...
    for w in LEG_Z_WINDOWS:
        m = log_p.rolling(w, min_periods=max(2, w // 2)).mean()
        s = log_p.rolling(w, min_periods=max(2, w // 2)).std()
        parts[f"legz{w}"] = (log_p - m) / s.replace(0, np.nan)
    parts["abs_r1"] = r1.abs()
...
def _spread_feats(log_a: pd.Series, log_b: pd.Series) -> pd.DataFrame:
    ...
    for w in PAIR_CORR_WINDOWS:
        parts[f"corr{w}"] = ra.rolling(w, min_periods=max(2, w // 2)).corr(rb)
    return pd.DataFrame(parts)
...
def _prepare() -> None:
    ...
    # Cross-sectional demeaned returns per date
    demean_ret: dict[int, pd.DataFrame] = {}
    for lag in DEMEAN_RET_LAGS:
        rL = log_p_all.diff(lag)
        demean_ret[lag] = rL.sub(rL.mean(axis=1), axis=0)
    ...
    for leg in legs:
        ...
        for lag in DEMEAN_RET_LAGS:
            xs_blocks[f"dret{lag}"] = demean_ret[lag][leg]
