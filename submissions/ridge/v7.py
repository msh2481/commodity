"""v7: rank-y training + more spread lags + cross-pair rank features.

Build on v6 (rank-transformed y, alpha=1) and add:
  - sret at all lags {2, 3, 4, 10, 40}
  - Cross-pair (across 374 unique spread pairs) rank of sret_L per date, L in {1, 5, 20}
"""
...
def _spread_feats(log_a: pd.Series, log_b: pd.Series,
                  pair_rank_blocks: dict[str, pd.Series]) -> pd.DataFrame:
    spread = log_a - log_b
    r1 = spread.diff(1)
    parts = {}
    ...
    for k, s in pair_rank_blocks.items():
        parts[k] = s
    return pd.DataFrame(parts)
...
def _prepare() -> None:
    ...
    # Precompute spread log-returns for each unique spread pair and rank them
    # cross-sectionally (rank per date across the set of unique spread pairs).
    spread_pair_strs = [p for p in pairs.pair.unique() if " - " in p]
    pair_spread_ret: dict[int, pd.DataFrame] = {}
    for lag in SPREAD_PAIR_RANK_LAGS:
        cols = {}
        for p in spread_pair_strs:
            a, b = [x.strip() for x in p.split(" - ")]
            cols[p] = log_p_by_leg[a].diff(lag) - log_p_by_leg[b].diff(lag)
        pair_spread_ret[lag] = pd.DataFrame(cols).rank(axis=1, pct=True) - 0.5
    ...
    for p in spread_pair_strs:
        ...
        pair_rank_blocks = {}
        for lag in SPREAD_PAIR_RANK_LAGS:
            pair_rank_blocks[f"ppr{lag}"] = pair_spread_ret[lag][p]
        pair_feats[p] = _spread_feats(log_p_by_leg[a], log_p_by_leg[b], pair_rank_blocks)
