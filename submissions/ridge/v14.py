"""v14: per-target feature selection + rank-y ridge.

For each target, select top-K features by absolute Pearson correlation with y_rank
on training data, then fit ridge on those K. K adaptive (top 30 of available).
"""
...
class Model:
    ...
    def fit(self, X: pd.DataFrame, y: pd.DataFrame) -> None:
        ...
        for t in self._target_cols:
            ...
            n_feats = Xf.shape[1]
            k = min(TOP_K, n_feats)
            # Rank features by |Pearson corr| with y
            yf_c = yf - yf.mean()
            y_std = yf_c.std()
            if y_std < 1e-12:
