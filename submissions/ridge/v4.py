"""v4: richer per-target features + cross-sectional ranks.

Over v3 adds:
  - lret at {2, 3, 4, 10, 40} (captures horizon structure for lag-1..4 targets)
  - xs-rank over all legs for lret at {1, 5, 20} and for vol at {20}
  - spread z-score at window 10 (short mean-reversion)
  - spread acceleration: sret1 - sret5/5
  - leg absolute return |lret1| (dispersion proxy)
"""
...
def _asset_feats(log_p: pd.Series, xs_rank_blocks: dict[str, pd.Series]) -> pd.DataFrame:
    r1 = log_p.diff(1)
    parts = {}
    for lag in RET_LAGS:
    ...
    parts["abs_r1"] = r1.abs()
    for key, series in xs_rank_blocks.items():
        parts[key] = series
