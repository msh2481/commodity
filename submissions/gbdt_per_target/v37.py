"""v37: v32 ensemble + structural K-selection (keep only lag>=3 targets).

v32 gets per-target rankings across folds with Spearman only ~0.15-0.22,
so data-driven top-K is too unstable. Structural selection (based on lag,
which is fixed per target) is leak-free and stable.

Diagnostic: v32 + mask lag>=3 (212 targets) → Sharpe 0.2255 (+0.006 vs v32).
"""
...
N_THREADS_FIT = 8
MIN_LAG = 3  # keep targets with lag >= this
...
def _build_jpx(X):
    ...
    j = prices[cols]
    j_logp = np.log(j.clip(lower=1e-9)); j_ret1 = j_logp.diff()
...
class Model:
    ...
    def __init__(self):
        ...
        self._X_train = None
        self._keep_idx: np.ndarray | None = None
    ...
    def fit(self, X, y):
        ...
        # Structural K: keep only targets with lag >= MIN_LAG
        self._keep_idx = np.array([j for j, t in enumerate(self._target_cols)
                                   if self._lag_of[t] >= MIN_LAG])
    ...
    def predict(self, X):
        ...
        # Structural K: keep targets in self._keep_idx, fill rest with per-day median
