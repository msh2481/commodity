"""v28: v15 + minimal JPX features (only Gold_Standard + Platinum_Standard).

JPX Gold_Standard (74 refs) and Platinum_Standard (71 refs) are 2 of the
top-6 assets in target pairs, after LME. Adding them to common tail
should help targets that involve these (≈30% of targets).

Common tail: 8 LME + 3 cal + 2 JPX raw + 2 JPX ret1 + 2 JPX vol20 = 17.
Plus per-target spread multi-window = 7 features.
Total: 24 (spread targets).
"""
...
def _build_common(X):
    ...
    jpx_cols_present = [c for c in JPX_MAIN if c in prices.columns]
    jpx = prices[jpx_cols_present]
    jpx_logp = np.log(jpx.clip(lower=1e-9))
    jpx_ret1 = jpx_logp.diff()
    jpx_vol20 = jpx_ret1.rolling(20, min_periods=10).std()
    jpx_block = pd.concat(
        [jpx, jpx_ret1.add_suffix("_ret1"), jpx_vol20.add_suffix("_vol20")],
        axis=1,
    ).fillna(0.0).astype(np.float32).values if jpx_cols_present else np.empty((len(Xs), 0), dtype=np.float32)
