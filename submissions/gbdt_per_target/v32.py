"""v32: weighted 4-way ensemble v15(2)+v17(2)+v28(3)+v1(1).

Diagnostic: 0.2191. v1's diversity adds fold-3 robustness.
All models use v1-like hparams (100 trees depth 4 lr 0.05).
"""
...
WEIGHTS = (2, 2, 3, 1)  # v15, v17, v28, v1
...
def _predict_quad(args):
    ...
    preds = []; ws = []
    if m15 is not None: preds.append(m15.predict(X15)); ws.append(WEIGHTS[0])
    if m17 is not None: preds.append(m17.predict(X17)); ws.append(WEIGHTS[1])
    if m28 is not None: preds.append(m28.predict(X28)); ws.append(WEIGHTS[2])
    if m1 is not None: preds.append(m1.predict(X1)); ws.append(WEIGHTS[3])
...
class Model:
    ...
    def fit(self, X, y):
        ...
        common_v1 = lme  # v1 feature set = just 4 LME raw + 4 ret1
