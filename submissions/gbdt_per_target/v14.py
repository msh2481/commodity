"""v14: v1 features + per-target spread features only.

Smallest possible upgrade to v1: add only spread (log A/B, dev20, z20)
per spread target. For single-asset targets, identical to v1.

Total: 11 features (v1's 8 + 3 spread) for spread targets, 8 for single.
v1 hparams.
"""
    ...
    def fit(self, X, y):
        ...
        for tgt in self._target_cols:
            ...
            if pair is not None and pair in spread_cache:
                X_j = np.hstack([lme, spread_cache[pair]])
            else:
                X_j = lme
