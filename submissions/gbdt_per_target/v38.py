"""v38: single-model baseline for tuning pass.

K-trick mask: predict only 71 targets with (lag>=4 AND has_LME).
Median-fill the other 353 with per-day median of the 71.

Feature set combines v15/v18/v28/v34 building blocks (~47 feats for spread targets).
Default XGBoost hparams (100 trees, depth 4, lr 0.05). This establishes
the baseline against which skopt will tune.
"""
...
def _build_common(X):
    """Common tail: LME extended + JPX main minimal + calendar."""
    Xs = X.sort_index()
    prices = Xs.ffill()

    # LME block (extended horizons)
    lme = prices[LME_COLS]
    lme_logp = np.log(lme.clip(lower=1e-9))
    lme_ret1 = lme_logp.diff()
    lme_ret5 = lme_logp - lme_logp.shift(5)
    lme_ret20 = lme_logp - lme_logp.shift(20)
    lme_vol20 = lme_ret1.rolling(20, min_periods=10).std()
    lme_block = pd.concat([
        lme,                                        # 4 raw
        lme_ret1.add_suffix("_ret1"),               # 4 ret1
        lme_ret5.add_suffix("_ret5"),               # 4 ret5
        lme_ret20.add_suffix("_ret20"),             # 4 ret20
        lme_vol20.add_suffix("_vol20"),             # 4 vol20
    ], axis=1).fillna(0.0).astype(np.float32).values

    # JPX main (Gold + Platinum Standard)
...
def _spread_features(logp_a, logp_b, lag_h):
    """Per-target spread features: multi-window + horizon-matched + spread vol."""
    spread = (logp_a - logp_b).astype(np.float32)
    s = pd.Series(spread)
    feats = [spread]  # current spread level
    for w in SPREAD_WINDOWS:
        ...
        feats.extend([dev, z])  # 6 feats (3 windows × 2)
    # Horizon-matched past returns of the spread
    sf = np.asarray(spread, dtype=np.float64)
    feats.append(_diff_n(sf, lag_h))
    feats.append(_diff_n(sf, 2 * lag_h))
    # Spread 1-day return vol
...
def _asset_min(logp):
    """4 features per asset: ret5, vol20."""
    n = len(logp)
...
class Model:
    device = "cuda"
    # Default hparams — tuned by skopt in subsequent versions
    HPARAMS = dict(
    ...
    def __init__(self):
        ...
        self._X_train = None
        self._keep_idx = None  # indices of targets we actually predict
    ...
    def fit(self, X, y):
        ...
        # Only fit models for kept targets; others stay None
    ...
    def predict(self, X):
        ...
        # K-trick: kept targets hold their predictions; others get per-day median of kept
        ...
        # Replace any remaining NaN (e.g. if all kept were NaN for a day) with 0
